			...
			if ( is_wp_error( $media_array ) ) {
				...
				<ul class="zozo_instagram_widget list-unstyled">
					<?php foreach($media_array as $item) {
						echo '<li class="instagram-item image-'. esc_attr( $size ) .'"><a href="'. esc_url( $item['link'] ) .'" target="'. esc_attr( $link_target ) .'"><img src="'. esc_url($item['thumbnail']) .'"  alt="'. esc_attr( $item['description'] ) .'" width="'. esc_attr( $item['dimensions']['width'] ) .'" height="'. esc_attr( $item['dimensions']['height'] ) .'" title="'. esc_attr( $item['description'] ).'"/></a></li>';
						}
					?>
				</ul>			
		<?php }
	...
	function scrape_instagram($user_name, $photos_count) {
		...
		if(false === ($instagram = get_transient('zozo-instagram-'.sanitize_title_with_dashes($user_name)))) {
			...
			if ( ! empty( $instagram ) ) {
				...
				set_transient( 'zozo-instagram-'.sanitize_title_with_dashes( $user_name ), $instagram, HOUR_IN_SECONDS * 2 );
			}
